...
<div class="container mt-4">
    ...
    <form class="form row mb-4" method="POST" action="" name="myForm" onsubmit="return(validate());">
        <?php
...
if (isset($_POST['simpan'])) {
    if (isset($_POST['id'])) {
        $ubah = mysqli_query($mysqli, "UPDATE kegiatan SET isi = '" . $_POST['isi'] . "', tgl_awal = '" . $_POST['tgl_awal'] . "', tgl_akhir = '" . $_POST['tgl_akhir'] . "' WHERE id = '" . $_POST['id'] . "'");
    } else {
        $tambah = mysqli_query($mysqli, "INSERT INTO kegiatan(isi, tgl_awal, tgl_akhir, status) VALUES ('" . $_POST['isi'] . "', '" . $_POST['tgl_awal'] . "', '" . $_POST['tgl_akhir'] . "', '0')");
    }
...
if (isset($_GET['aksi'])) {
    if ($_GET['aksi'] == 'hapus') {
        ...
        $ubah_status = mysqli_query($mysqli, "UPDATE kegiatan SET status = '" . $_GET['status'] . "' WHERE id = '" . $_GET['id'] . "'");
    }
    echo "<script>document.location='index.php';</script>";
}
?>
